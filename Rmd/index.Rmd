...
• We used the same method as for the Ebola Lancet piece, which estimated a corrected CFR rather than using a naive cases/deaths calculation. As a starting point we assumed a 13.1 day onset-to-death for nCoV (onset-to-hospitalisation for recent cases was 9.1 days in the Li et al NEJM paper, then we added 4 days to this). Distribution shown in Figure 1A. Severity was similarly estimated using a delay 9.1 days

• We only have decent total incidence estimates from 16th Jan onwards (Figure 2B), so if the delay is ~13 days, we're now just getting to the point where we can calculate outcomes from this period (because otherwise the death outcomes will include cases prior to 16th Jan). 
...
knitr::include_graphics("../plots/cfr_estimate_scale_65_mean_13.1_2020-02-01.png")
```
