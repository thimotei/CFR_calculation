...
output_estimates <- function(main_data_in, delay_fun)
{
  ...
  CI_nCFR_raw <- c.input.text(bin_conf(data_1[tt_max,]$deaths,data_1[tt_max,]$cases))
  CI_nCFR <- c.input.text(bin_conf(data_1[tt_max,]$deaths,round(scaled_reporting*data_1[tt_max,]$cases)))
  CI_cCFR <- c.input.text(bin_conf(store_cfr$deaths[tt_max], round(scaled_reporting*(store_cfr$known_outcomes[tt_max]))))
