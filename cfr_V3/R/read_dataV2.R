# new data
southKoreaData <- read.csv("data/south_korea/south_korea.csv")
international_data_with_cruise <- read.csv("data/international_data//international_data_by_confirmation.csv")
international_data_without_cruise <- read.csv("data/international_data/international_data_by_confirmation_without_cruise.csv")
...
#internationalDataRaw <-  data.frame(read.csv("~/repos/CFR_calculation/cfr_V2/data/international_cases_deaths.csv"))
#cruiseShipDataRaw <-  data.frame(read.csv("~/repos/CFR_calculation/cfr_V2/data/cruise_ship_diamond_princess_by_confirmation.csv"))
#christiansData <- read.csv("~/repos/CFR_calculation/cfr_V2/ncov_cases_20200217.csv")
#bno_news_data <- read.csv("~/repos/CFR_calculation/cfr_V2/data/bno_news_cases_deaths_2020-27-02.csv")


# changing dates from factors to dates
southKoreaData$date <- as.Date(southKoreaData$date)
international_data_with_cruise$date <- as.Date(international_data_with_cruise$date)
international_data_without_cruise$date <- as.Date(international_data_without_cruise$date)
...
inferredDataIncidence <- read.csv("data/wuhan_data/inferred_data.csv")
