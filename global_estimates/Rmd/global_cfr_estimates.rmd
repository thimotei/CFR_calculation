...
* The adjusted CFR does not account for under-reporting. However, given that the best available estimates of CFR (adjusting or controlling for under-reporting) are in the 1%-1.5% range, we therefore assume that the baseline CFR is 1.1% (95% CI: 0.3–2.4%)[5], and if a country has an adjusted CFR that is higher (e.g. 20%), it suggests that only a fraction of cases have been reported (in this case, $\frac{1.1}{20} = 5.5%$ cases reported approximately).

## Current estimates for percentage of symptomatic cases reported for countries with greater than five deaths {.tabset}
...
dataTable <- allTogetherClean %>% dplyr::select(country, under-reporting_estimate, total_cases, total_deaths)
...
```{r include = TRUE, fig.width = 13, fig.height = 12}
  ...
  ggplot2::geom_linerange(ggplot2::aes(ymin = bottom, ymax = top), col = "#344b85", size = 4,  alpha = 0.3) +
  ggplot2::geom_linerange(ggplot2::aes(ymin = lower, ymax = upper), col = "#344b85",  size = 4,  alpha = 0.5) +
  ggplot2::scale_y_continuous(labels = scales::percent_format(accuracy = 1L), limits=c(0,1)) +
...
>>>>>>> 8292c5a4d75761ae770bc35ab0a6e98011c5c2c4
...
At this stage, raw estimates of the CFR of COVID-19 correcting for delay to outcome, but not under-reporting, have been calculated. These estimates range between 1% and 1.5% [5, 6, 7]. We assume a CFR of 1.1% [5] - calculated in a population where very little under-reporting was possible as every individual was tested - as a baseline CFR. We use it to approximate the potential level of under-reporting in each country. Specifically, we perform the calculation $\frac{1.1}{\text{cCFR}}$ of each country to estimate an approximate fraction of cases reported.

## Limitations

Implicit in assuming that the under-reporting is 1.1/cCFR for a given country is that the deviation away from the assumed 1.1% CFR is entirely down to under-reporting. In reality, burden on healthcare system is a likely contributing factor to higher than 1.1% CFR estimates, along with many other country specific factors.
