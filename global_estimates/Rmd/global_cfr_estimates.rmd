...
*Timothy W Russell<sup> &ast; </sup>, Joel Hellewell<sup>†</sup>, Christopher I Jarvis<sup>†</sup>, Kevin Van Zandvoort<sup>†</sup>, Sam Abbott<sup>†</sup>, Ruwan Ratnayake<sup></sup>, CMMID COVID-19 working group, Stefan Flasche<sup></sup>, Rosalind M Eggo<sup></sup>, W John Edmunds<sup></sup>, Adam J Kucharski<sup></sup>*
...
* The adjusted CFR does not account for under-reporting. However, given that the best available estimates of CFR are in the 1-1.5% range, we therefore assume that the baseline CFR is 1%, and if a country has an adjusted CFR that is higher (e.g. 20%), it suggests that only a fraction of cases have been reported (in this case, 1/20).

## Current estimates for percentage of symptomatic cases reported and naive and delay-corrected fatality ratios {.tabset}

### Figure

```{r include = FALSE}
allTogetherClean <- readRDS("../data/all_together_plot_clean.rds")
 allTogetherOrdered <- allTogetherClean %>% 
    dplyr::mutate(
      country = country  %>% 
...
```{r include = TRUE, fig.width = 13, fig.height = 20}
  allTogetherOrdered %>% 
  ...
  ggplot2::facet_wrap(~ confidence, ncol = 1, scales = "free", shrink = TRUE) +
  cowplot::theme_cowplot() +
  cowplot::panel_border() +
  ggplot2::theme(axis.text.x = element_text(angle = 0, size = 10), axis.text.y = element_text(size = 10)) +
  ggplot2::theme(legend.position = "none") +
  ggplot2::guides(col = ggplot2::guide_legend(nrow = 2)) +
  ggplot2::labs(x = "Countries", y = "Percentage of cases reported") + 
  ggplot2::coord_flip()
```
*Figure 1: Plotting the estimates for the proportion of symptomatic cases reported in different countries using cCFR estimates. Dark blue shading is the 25% - 75% confidence range and light blue is 2.5% - 97.5% confidence range*

### Table

```{r include = FALSE}
allTogether <- readRDS("../data/all_results_reporting_cases_deaths")
```
...
*Table 1: Estimates for the proportion of symptomatic cases reported in different countries using cCFR estimates based on case and death timeseries data from the ECDC. Total cases and deaths in each country is also shown. Confidence intervals calculated using an exact binomial test with 95% significance. Naive estimates are simply deaths divided by cases.*
