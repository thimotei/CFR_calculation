...
allTogetherClean2 <- allDatDesc %>%
  ...
  dplyr::mutate(nCFR_UQ = binom.test(total_deaths, total_cases)$conf.int[2],
                ...
                cCFR_UQ = binom.test(total_deaths, cum_known_t)$conf.int[1],
                cCFR_LQ = binom.test(total_deaths, cum_known_t)$conf.int[2],
                underreporting_estimate = cCFRBaseline / (100*cCFR),
                lower = cCFREstimateRange[1]/(100 * cCFR_LQ),
                upper = cCFREstimateRange[2]/(100 * cCFR_UQ),
