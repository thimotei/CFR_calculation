...
for(ii in 1:length(countrypickList)){
  ...
  data_fit <- cbind(xx_date,reporting_estimate,cases_known,data_1_in_Pick$new_deaths)
  data_fit <- as_tibble(data_fit); names(data_fit) <- c("date","report","cases_known","deaths")
  ...
  modelB.P <- gam(deaths ~ s(date) + offset(log_offset) , data = data_fit,family = "poisson")
